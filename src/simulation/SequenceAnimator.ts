...
export class SequenceAnimator {
    ...
    private targetDropPositions: THREE.Vector3[] = [];
    ...
    init(mjModel: MujocoModel, isStacking: boolean, getName: (addr: number) => string) {
        ...
        for (let i = 0; i < mjModel.nbody; i++) {
            ...
            if (n === 'stack_base' || n === 'delivery_box' || (!isStacking && n === 'tray')) this.trayId = i;
        }
    }

    /**
     * Start the sequence.
     * @param targets Can be an array of body IDs (number[]) OR an object with { positions, targetPositions, markerIds } for blind mode.
     */
    start(ikTarget: THREE.Object3D, mjData: MujocoData, ikSystem: IkSystem, 
          targets?: { positions: THREE.Vector3[], targetPositions?: THREE.Vector3[], markerIds: number[] } | number[], 
        ...
        if (targets && !Array.isArray(targets) && 'positions' in targets) {
             ...
             this.targetDropPositions = targets.targetPositions || [];
    ...
    prepareStep(ikTarget: THREE.Object3D, mjData: MujocoData, ikSystem: IkSystem) {
         ...
         if (this.useLivePosition && this.curCubeIdx < this.cubeIds.length) {
             ...
             // We lower it by 2cm.
             // cPos.z -= 0.02; // Removed for more explicit control from agent
         }

         // Get tray position
         let tPos = new THREE.Vector3(mjData.xpos[this.trayId*3], mjData.xpos[this.trayId*3+1], mjData.xpos[this.trayId*3+2]);
         
         if (!this.useLivePosition && this.curCubeIdx < this.targetDropPositions.length) {
             tPos.copy(this.targetDropPositions[this.curCubeIdx]);
         }
         ...
         if (this.isStacking && this.useLivePosition) {
