      ...
      <div className={`p-8 border-b flex justify-between items-center ${headerBorder}`}>
        <div className="flex items-center gap-4 w-full">
          <div className="w-full pr-4">
            <h2 className="text-lg font-bold leading-none mb-3 tracking-tight">Aegis Command</h2>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-6 pt-1.5 pb-6 space-y-2">
        ...
        <section className="space-y-1.5">
          <div className={`p-1.5 rounded-2xl flex border ${selectorBg}`}>
              {(['2D bounding boxes', 'Segmentation masks', 'Points'] as DetectType[]).map((t) => {
                ...
                const isItemDisabled = isSeg && (modelId === 'gemini-3-flash-preview' || modelId === 'gemini-3.1-pro-preview');
                ...
                return (
                  <button 
                    ...
                    onClick={() => setType(t)}
                    title={
                      t === '2D bounding boxes' ? 'Identify disaster zones and assets' : 
                      t === 'Segmentation masks' ? 'Calculate precise impact boundaries' : 
                  ...
                  >
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-tight">
                      {t === '2D bounding boxes' ? 'Zones' : t === 'Segmentation masks' ? 'Impact' : 'Targets'}
                    </span>
                  </button>
                );
              })}
          ...
          {isInvalidConfig && (
             <div className={`p-3 rounded-xl border text-[10px] leading-relaxed animate-in slide-in-from-top-1 fade-in duration-200 ${isDarkMode ? 'bg-amber-500/10 border-amber-500/20 text-amber-200' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>
                <p>
                  <span className="font-bold">Advanced Analysis:</span> High-precision segmentation is currently optimized for specialized models. Standard reasoning models will use bounding boxes for zone identification.
                </p>
             </div>
          )}
        </section>
        ...
        <section className="space-y-2">
          <div className="relative group">
            <textarea 
                ...
                placeholder="Identify disaster impact..."
            />
            <button
              ...
              title="Toggle Intelligence Settings"
            >
          ...
          {showSettings && (
            <div className={`flex items-center justify-between gap-4 px-1 py-1 animate-in slide-in-from-top-2 fade-in duration-200 ${isDarkMode ? 'bg-slate-800/20 rounded-xl' : 'bg-slate-50/50 rounded-xl'}`}>
               ...
               <div className="flex items-center gap-2 pt-4 p-2" title="Thinking improves the capabilities of the model to reason through complex disaster scenarios.">
                   <input 
                     type="checkbox" 
                     id="thinking-toggle"
          ...
          <div className="flex gap-3">
            <button 
                ...
                title="Analyze: Trigger Gemini analysis of current disaster zone"
            ...
            <button 
                ...
                title={isPickingUp ? `Click to increase simulation speed (Current: ${playbackSpeed}x)` : "Deploy AEGIS Response to identified targets"}
            ...
            >
                {isPickingUp ? (
                  <>
                    ...
                    <span>Speed Up {playbackSpeed > 1 ? `(${playbackSpeed}x)` : ''}</span>
                  </>
                ) : (
                  <>
        ...
        <section className="space-y-2 pt-2">
          <div className="flex items-center gap-2 px-1">
            ...
            <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Mission Log</h3>
          </div>
