...
import { CapsuleGeometry } from '../src/simulation/CapsuleGeometry';
import { Reflector } from '../src/simulation/Reflector';
...
export class GeomBuilder {
    ...
    create(mjModel: MujocoModel, g: number): THREE.Object3D | null {
        ...
        if (geo) {
            let mesh;
            // Special handling for the floor plane to make it look like polished concrete
            if (type === getVal(MG.mjGEOM_PLANE)) {
                mesh = new Reflector(geo, {
                    clipBias: 0.003,
                    textureWidth: 1024, textureHeight: 1024,
                    color: new THREE.Color(0x888888), // Concrete gray
                    mixStrength: 0.1 // Subtle reflection
                });
            } else {
                ...
                mesh = new THREE.Mesh(geo, new THREE.MeshStandardMaterial({
                    ...
                    opacity,
                    roughness: 0.4, // Slightly smoother for metallic parts
                    metalness: 0.5  // More metallic feel
                }));
                // Enable shadows
